""" I just copied code from authors solution, 
    but added function that takes any string 
    and draws it using functions below """
...
def draw_text(bob, text, size):
    """"draw any str passed to a func. with turtle and above functions
    bob - turtle object
    text - str(lowered in the func.)
    size - int
    
    """
    lt_letters = list(text.lower())
    temp = []
    for letter in lt_letters:
        temp.append('draw_' + letter)
...
if __name__ == '__main__':
    ...
    size = 20
    bob = turtle.Turtle()
    draw_text(bob, 'hackerman indeed', size)
    turtle.mainloop()
